...
def main(interval):
    ...
    model_theta = "HCI_10_S"
    pth_path_root = "../Train_M/Record/" + model_theta + "/model_best.pth"
    save_root = "Record/Intaglio_R/" + model_theta + "/" + str(interval) + "/"
    check_and_create_folder(save_root)
    model = nn.DataParallel(model, device_ids=[0])
    checkpoint = torch.load(pth_path_root)
    model.load_state_dict(checkpoint, strict=True)
    data_path_root = "/mnt/data2/zhangjiangfeng/zjf_data/PublicDataset/deal_Intaglio/"
    # 总采样数
    sample = 10

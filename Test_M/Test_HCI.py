...
def main(interval):
    ...
    save_root = "Record/HCI_10_S/" + str(interval) + "/"
    data_root = "/mnt/data2/zhangjiangfeng/zjf_data/PublicDataset/4D-Light-Field-Dataset/HCI_FS_trainval_10.h5"
    ...
    # 总采样数
    sample = 10
    test_interval, test_index = sample_index(sample, interval)
    Dataset = HCI_dataset(data_root, sample, "stack_val", "disp_val")
    ...
    with torch.no_grad():
        ...
        for idx, samples in enumerate(tqdm(dataloader, desc="Test")):
            test_input, test_gt_depth, test_focus_dists, test_mask = samples
            test_input = test_input[:, :, ::test_interval, :, :].cuda()
            test_focus_dists = test_focus_dists[:, test_index, ...].cuda()
            ...
            Avg_mse = Avg_mse + mask_mse(pred, test_gt_depth, test_mask)
            Avg_mae = Avg_mae + mask_mae(pred, test_gt_depth, test_mask)
